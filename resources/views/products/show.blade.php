...
<section class="inner-section section-shell product-detail">
    ...
    <div class="product-info">
        ...
        <form action="{{ route('cart.store', $product) }}" method="POST" class="buy-form" data-ajax-cart>@csrf<label>Quantity<input type="number" name="quantity" min="1" max="100" value="1"></label><button type="submit" class="button button-primary" @disabled(!$product->in_stock)><span class="button-icon"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 4h2l2 10h10l2-7H7M10 19a1 1 0 1 1-2 0 1 1 0 0 1 2 0Zm8 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0Z"/></svg></span><span data-button-label>Add to Cart</span></button><a href="tel:+92{{ ltrim($siteSettings['site_phone'] ?? '03226780242', '0') }}" class="button outline-button">Call to order</a><button type="button" class="button outline-button share-detail-button" data-share-product data-share-title="{{ $product->name }}" data-share-text="View {{ $product->name }} on KISANWORLD" data-share-url="{{ route('products.show', $product) }}"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><path d="M8.6 10.6 15.4 7.4M8.6 13.4l6.8 3.2"/></svg><span>Share</span></button></form>
